...
- (void)__expr {
    ...
    while ([self predicts:MINIMATH_TOKEN_KIND_PLUS, 0]) {
        if ([self speculate:^{ if (self.enableAutomaticErrorRecovery && LA(1) != MINIMATH_TOKEN_KIND_PLUS && LA(2) == MINIMATH_TOKEN_KIND_PLUS) {[self consume:LT(1)];}[self match:MINIMATH_TOKEN_KIND_PLUS]; [self discard:1];[self mult]; [self execute:(id)^{ PUSH_FLOAT(POP_FLOAT()+POP_FLOAT()); }];}]) {
            if (self.enableAutomaticErrorRecovery && LA(1) != MINIMATH_TOKEN_KIND_PLUS && LA(2) == MINIMATH_TOKEN_KIND_PLUS) {
                [self consume:LT(1)];
            }
...
- (void)__mult {
    ...
    while ([self predicts:MINIMATH_TOKEN_KIND_STAR, 0]) {
        if ([self speculate:^{ if (self.enableAutomaticErrorRecovery && LA(1) != MINIMATH_TOKEN_KIND_STAR && LA(2) == MINIMATH_TOKEN_KIND_STAR) {[self consume:LT(1)];}[self match:MINIMATH_TOKEN_KIND_STAR]; [self discard:1];[self pow]; [self execute:(id)^{ PUSH_FLOAT(POP_FLOAT()*POP_FLOAT()); }];}]) {
            if (self.enableAutomaticErrorRecovery && LA(1) != MINIMATH_TOKEN_KIND_STAR && LA(2) == MINIMATH_TOKEN_KIND_STAR) {
                [self consume:LT(1)];
            }
...
- (void)__pow {
    ...
    [self atom]; 
    if ([self speculate:^{ if (self.enableAutomaticErrorRecovery && LA(1) != MINIMATH_TOKEN_KIND_CARET && LA(2) == MINIMATH_TOKEN_KIND_CARET) {[self consume:LT(1)];}[self match:MINIMATH_TOKEN_KIND_CARET]; [self discard:1];[self pow]; [self execute:(id)^{ 		double exp = POP_FLOAT();		double base = POP_FLOAT();		double result = base;	for (NSUInteger i = 1; i < exp; i++) 			result *= base;		PUSH_FLOAT(result); 	}];}]) {
        if (self.enableAutomaticErrorRecovery && LA(1) != MINIMATH_TOKEN_KIND_CARET && LA(2) == MINIMATH_TOKEN_KIND_CARET) {
            [self consume:LT(1)];
        }
