...
//    TDNotNil(rootNode);
//    TDEqualObjects(@"(@start:SEQ (foo:SEQ (bar:SEQ (:| (:SEQ (baz:SEQ :Word)) (:SEQ (bat:SEQ :Number))))))", [rootNode treeDescription]);
...
- (void)testAlternationAST2 {
    ...
    PKAST *rootNode = [_factory ASTFromGrammar:g simplify:NO error:&err];
    TDNotNil(rootNode);
    TDEqualObjects(@"(@start:SEQ (foo:SEQ (:| (:SEQ :Word) (:SEQ :Number))))", [rootNode treeDescription]);
